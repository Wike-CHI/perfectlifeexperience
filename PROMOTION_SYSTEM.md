...
**层级定义：**
*   **0级 (总公司)**: 系统保留，不可晋升。
*   **1级 (一级代理)**: 最高代理等级，享受 20% 基础佣金。
*   **2级 (二级代理)**: 享受 15% 基础佣金。
*   **3级 (三级代理)**: 享受 10% 基础佣金。
*   **4级 (四级代理)**: 初始等级，享受 5% 基础佣金。
...
1.  **基础佣金 (Commission)**
    *   **规则**: 根据上级代理的 `agentLevel` 确定比例。
    *   **比例**: 1级(20%) > 2级(15%) > 3级(10%) > 4级(5%)。
    *   **逻辑**: 保持原有的多级分润逻辑，按距离由近及远匹配。
...
2.  **拨比监控**: 虽然系统有级差保护，但建议定期通过 `reward_records` 分析总拨比（Total Payout Ratio），确保在商品利润覆盖范围内。
3.  **跳级通道**: 目前实现了“四级 -> 二级”的底层逻辑支持，未来可配合运营活动开放“连续金牌直接跳级”的自动处理逻辑。
