...
// ==================== 晋升门槛常量 ====================
// 晋升规则：
// - 四级→三级（普通→铜牌）：累计销售额 >= 2万元
// - 三级→二级（铜牌→银牌）：本月销售额 >= 5万元 或 团队人数 >= 50人
// - 二级→一级（银牌→金牌）：本月销售额 >= 10万元 或 团队人数 >= 200人
//
// ⚠️ 重要：金额单位为「分」
// - 数据库和管理端统一使用「分」存储
// - 管理端显示时转换为「元」（除以100）
...
//
// 默认值说明：
// - 2000000 分 = 20000 元 = 2万元
// - 5000000 分 = 50000 元 = 5万元
// - 10000000 分 = 100000 元 = 10万元

const PromotionThreshold = {
  // 四级 → 三级（普通 → 铜牌）
  LEVEL_4_TO_3: {
    totalSales: 2000000      // 累计销售额 >= 2万元（单位：分）
  },

  // 三级 → 二级（铜牌 → 银牌）
  LEVEL_3_TO_2: {
    monthSales: 5000000,     // 本月销售额 >= 5万元（单位：分）
    teamCount: 50            // 或团队人数 >= 50人
  },

  // 二级 → 一级（银牌 → 金牌）
  LEVEL_2_TO_1: {
    monthSales: 10000000,    // 本月销售额 >= 10万元（单位：分）
